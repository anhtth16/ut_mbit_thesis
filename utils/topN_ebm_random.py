...
def topN_ebm_extended(model, N, ranking_data, user_fm, job_fm): # features_df should be imported before running this script
...
#     mark_stop = 0
    for u_id in user_ids:
        ...
        for j_id in job_ids:
            ...
            user = user_fm[user_fm.UserID==u_id].reset_index()[features_user].loc[0]
    
            # Get job details
            job_details = job_fm[job_fm.JobID==j_id] \
                        .reset_index()[features_job] \
                        .loc[0]
...
def topN_ebm_side_info(model, N, ranking_data, user_fm, job_fm): # features_df should be imported before running this script
    user_ids = list(ranking_data.groupby('UserID').UserID.unique().astype('int'))
    rec_result = {}
#     mark_stop = 0
    for u_id in user_ids:
